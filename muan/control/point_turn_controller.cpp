#include "muan/control/point_turn_controller.h"

namespace muan {
namespace control {

PointTurnController::PointTurnController(DrivetrainModel model, double beta,
                                         double zeta)
    : model_(model), beta_(beta), zeta_(zeta) {}

PointTurnController::Output PointTurnController::Update(
    Eigen::Vector2d velocity, Eigen::Vector2d acceleration, Pose current,
    Pose error, bool high_gear) {
  double k1 = 2. * zeta_ * std::sqrt(beta_ * velocity(0) * velocity(0) +
