...
TEST(SplineGenerator, OneSpline) {
  ...
  std::vector<PoseWithCurvature> samples = generator.ParametrizeSpline(spline);
...
TEST(SplineGenerator, TwoSplines) {
  SplineGenerator generator;
  Pose initial_pose_a =
      Pose((Eigen::Matrix<double, 3, 1>() << 0, 0, M_PI / 2.).finished());
  Pose final_pose_a =
  ...
  std::vector<PoseWithCurvature> samples =
      generator.ParametrizeSplines(splines);
  PoseWithCurvature previous = samples.at(0);
