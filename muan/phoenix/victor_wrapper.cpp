...
void VictorWrapper::LoadConfig(Config config) {
  ...
  victor_.ConfigClosedloopRamp(config.closed_loop_ramp_time,
                               kVictorSetupTimeout);
...
void VictorWrapper::SetGains(Gains gains, int slot) {
  ...
  victor_.Config_kI(slot,
                    gains.i,  // * kVictorOutput * conversion_factor_ / ms,
                    kVictorSetupTimeout);

  victor_.Config_kD(slot,
                    gains.d,  // * kVictorOutput * conversion_factor_ * ms,
                    kVictorSetupTimeout);
...
void VictorWrapper::SelectGains(int slot) {
  victor_.SelectProfileSlot(slot, 0);
}
