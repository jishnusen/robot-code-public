 ...
 public:
  void Update() {
    ...
    if (calibrate_test_) {
      ...
      arm_input_proto_->set_arm_hall(
          std::abs(arm_status_proto_->arm_angle() - kHallEffectAngle) < 2e-2);
    }
    WriteMessages();
    arm_.Update();
    ReadMessages();
  ...
  void CalibrateDisabled(double offset = 0) {
    ...
    for (int i = 0; i < 2000; i++) {
      ...
      arm_input_proto_->set_arm_encoder(h);
      Update();
    }
    EXPECT_TRUE(arm_status_proto_->arm_calibrated());
    EXPECT_NEAR(arm_status_proto_->arm_angle(), 1., 1e-3);
  }
...
TEST_F(ArmTest, EncoderFault) {
  ...
  for (int i = 0; i < 400; i++) {
    arm_input_proto_->set_arm_encoder(0);
    arm_input_proto_->set_arm_voltage(12.);
    WriteMessages();
    arm_.Update();
    ReadMessages();
  }

  EXPECT_TRUE(arm_status_proto_->arm_encoder_fault());
  EXPECT_NEAR(arm_status_proto_->arm_angle(), 0, 1e-3);
}
...
TEST_F(ArmTest, IntakeModes) {
  ...
  has_cube_ = true;
  /* RunFor(1); */
  /* EXPECT_FALSE(arm_status_proto_->has_cube()); */
  /* CheckGoal(); */
