...
void DriveStraight::LeftRocket() {
  ...
  QueueManager<DrivetrainStatus>::Fetch()->ReadLastMessage(&drive_status);
  SetFieldPosition(6.4, 3.5, (drive_status->estimated_heading() - init_gyro) + init_theta);
...
void DriveStraight::CargoShip() {
  ...
  GoTo(superstructure::HATCH_ROCKET_FIRST, superstructure::PREP_SCORE);
  StartDrivePath(7.2, 1.3, -85 * (M_PI / 180), 1, true); // 1.0 m extra intial distance; lvl 2
  ...
  QueueManager<DrivetrainStatus>::Fetch()->ReadLastMessage(&drive_status);
  SetFieldPosition(7.25, 1.1, (drive_status->estimated_heading() - init_gyro) + init_theta); // At right 2nd CS
...
success = StartDriveVision();
  ...
  StartDrivePath(7.5, 2.7, -90 * (M_PI / 180), -1, true); // 1.0 m extra intial distance; lvl 2
  WaitUntilDriveComplete();
  Wait(200);
  ExitAutonomous();  // bye
}
...
void DriveStraight::CargoRocket() {
  ...
  GoTo(superstructure::HATCH_ROCKET_FIRST, superstructure::PREP_SCORE);
  StartDrivePath(5., .3, 0 * (M_PI / 180.), 1, true, false, 2.0);
  WaitUntilDriveComplete();
  /* WaitUntilDrivetrainNear(4.3, .6, .6); */
  /* StartDriveVision(0.78); */
  /* ScoreHatch(1); */
  /* Wait(50); */
 
  /* QueueManager<DrivetrainStatus>::Fetch()->ReadLastMessage(&drive_status); */
  /* SetFieldPosition(5.2, .3, (drive_status->estimated_heading() - init_gyro) + init_theta); */
  /* GoTo(superstructure::HATCH_SHIP_FORWARDS, superstructure::INTAKE_HATCH); */
  /* StartDrivePath(1.6, 3.3, 0., -1, true); */
  /* WaitUntilDriveComplete(); */
  //  StartPointTurn(M_PI);
  /*success = StartDriveVision();
  if (!success) {
