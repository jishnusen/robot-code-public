...
void Rocket::RightBackRocket() {
  ...
  WaitUntilDriveComplete();
  final_vel_ = 1.5;
  StartDrivePath(1., -3.4, -170 * (M_PI / 180.), 1, true, true, 0, 0.5);
  GoTo(superstructure::HATCH_ROCKET_FIRST, superstructure::INTAKE_HATCH);
  final_vel_ = 0.0;
  /* final_vel_ = 0.0; */
  /* Wait(150); */

  WaitUntilDrivetrainNear(1., -3.4, 0.3);
