...
void Rocket::LeftCargoRocket() {
  ...
  GoTo(superstructure::CARGO_GROUND, superstructure::PREP_SCORE);
  StartDrivePath(5., .3, 0., 1, Gear::kHighGear, 2.0);
  ...
  StartDrivePath(7.4, 2.8, 220 * (M_PI / 180.), -1);
...
void Rocket::RightRocket() {
  ...
  StartDrivePath(7.4, -3.1, -210 * (M_PI / 180.), -1);
  GoTo(superstructure::CARGO_GROUND, superstructure::PREP_SCORE);
  ...
  StartDrivePath(7.1, -2.1, -150. * (M_PI / 180.), -1);

  WaitUntilDriveComplete();
  StartDrivePath(.4, -3.3, -M_PI, 1, Gear::kHighGear, 0, 0.5);
  Wait(150);
  ...
  StartDrivePath(4.6, -2., 0. * (M_PI / 180.), -1);
  Wait(25);
