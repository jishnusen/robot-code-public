...
using LimeGoal = muan::lime::LimeGoalProto;
...
TeleopBase::TeleopBase()
  ...
  vision_tracking_ = throttle_.MakeButton(1);
...
void TeleopBase::SendDrivetrainMessage() {
  ...
  LimeGoal limegoal;
  LimeStatus limestatus;

  status_reader_.ReadLastMessage(&limestatus);
  ...
  limegoal->set_light_state(muan::lime::LightState::OFF);
  if (vision_tracking_->is_pressed()) {
    limegoal->set_light_state(muan::lime::LightState::ON);
  }

  if (vision_tracking_->is_pressed() && limestatus->has_target()) {
    wheel += limestatus->theta() * -0.03;
    quickturn = true;
  }
  ...
  QueueManager<LimeGoal>::Fetch()->WriteMessage(limegoal);
}
